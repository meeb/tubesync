...
# ======================================================================
# DEPENDENCIES:
# This utility requires only 'perl-base' and the following binaries
# which are standard in Debian 12/13 base images:
#   - /usr/bin/cksum (from coreutils) for integrity verification
#   - /usr/bin/gzip  (for compressed patch support)
# ======================================================================

# --- 1. Global Constants & Indices ---
# Stat array indices for readability and easy maintenance
my ($ST_MODE, $ST_UID, $ST_GID, $ST_SIZE, $ST_MTIME) = (2, 4, 5, 7, 9);
...
if ($dry_run) {
    ...
    my ($ready_cnt, $fail_cnt, $new_cnt, $del_cnt) = (0, 0, 0, 0);
    
    foreach my $f (sort keys %patches) {
        my $abs_target = -e $f ? abs_path($f) : $f;
        
        if ($patches{$f}{deleted}) {
            print "DELETE: [Target Removal]\n  Path: $abs_target\n\n";
            $ready_cnt++; $del_cnt++; next;
        }
        ...
        if (-f $f) {
            ...
            if (exists $state_metadata{$f} && $state_metadata{$f}{hash} ne "" 
                && $state_metadata{$f}{hash} ne $file_hash) {
                print "FAIL:   [Hash Conflict] (Content changed since state recorded)\n";
                print "  Path: $abs_target\n";
                my $oct_mode = sprintf("%05o", $stats[$ST_MODE] & 07777);
                print "  Stat: Size:$stats[$ST_SIZE] UID:$stats[$ST_UID] GID:$stats[$ST_GID] Mode:$oct_mode\n\n";
                $fail_cnt++;
                next; 
            }
            ...
            my (@offsets, $failed, $matched_hunks) = ((), 0, 0);
            ...
            my $total = scalar @{$patches{$f}{hunks}};
            if ($failed) {
                print "FAIL:   ($matched_hunks/$total hunks matched)\n";
                $fail_cnt++;
            } else {
                # Report if we found it exactly or had to use fuzz
                my $drift = 0; foreach (@offsets) { $drift = abs($_) if abs($_) > $drift; }
                my $type = $drift == 0 ? "Exact" : "Fuzzed (drift $drift)";
                print "READY:  [$type] ($matched_hunks/$total hunks)\n";
                $ready_cnt++;
            }
            ...
            print "  Path: $abs_target\n";
            print "  Stat: # File does not yet exist\n\n";
    ...
    if ($fail_cnt > 0) {
        ...
        print "  $fail_cnt file(s) could not be matched.\n";
        exit 1;
    } else {
        print "VERDICT: SUCCESS\n";
        print "  $ready_cnt file(s) ready to patch.\n";
        print "  $new_cnt file(s) pending creation.\n" if $new_cnt > 0;
        print "  $del_cnt file(s) pending removal.\n" if $del_cnt > 0;
        exit 0;
    }
}
...
eval {
    foreach my $target (keys %patches) {
        ...
        if (-e $backup_file) {
            my @orig_stat = stat($backup_file);
            # Apply original UID and GID
            chown($orig_stat[$ST_UID], $orig_stat[$ST_GID], $temp_work_file)
                or warn "$me: Warning: Could not preserve ownership for $target: $!\n";
            
            # Apply original permissions (mode)
            # Mask with 07777 to keep only the permission and special bits
            chmod($orig_stat[$ST_MODE] & 07777, $temp_work_file)
                or warn "$me: Warning: Could not preserve permissions for $target: $!\n";
        }
