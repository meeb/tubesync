...
class ModifyChaptersPP(FFmpegPostProcessor):
    ...
    def run(self, info):
        ...
        if not chapters:
            chapters = [
                {
                    "start_time": 0,
                    "end_time": info.get("duration") or real_duration,
                    "title": info["title"],
                }
            ]
        ...
        self.write_debug(
            "Concat spec = {}".format(
                ", ".join(
                    f'{c.get("inpoint", 0.0)}-{c.get("outpoint", "inf")}'
                    for c in concat_opts
                )
            )
        )
        ...
        in_out_files.extend(
            remove_chapters(in_file, True) for in_file in self._get_supported_subs(info)
        )
    ...
    def _mark_chapters_to_remove(self, chapters, sponsor_chapters):
        if self._remove_chapters_patterns:
            ...
            for c in chapters:
                if any(
                    regex.search(c["title"]) for regex in self._remove_chapters_patterns
                ):
                    c["remove"] = True
                    warn_no_chapter_to_remove = False
        ...
        while chapters:
            ...
            # (sponsor/normal, cut).
                ...
                if "_categories" in cur_chapter:
                    after_c = dict(
                        cur_chapter, start_time=c["end_time"], _categories=[]
                    )
                    cur_cats = []
            ...
            # (normal, sponsor) and (sponsor, sponsor)
                ...
                # Push the part after the overlap to PQ.
                    ...
                    after_cur = dict(
                        copy.deepcopy(c), start_time=cur_chapter["end_time"]
                    )
                    heapq.heappush(
    ...
    def _remove_tiny_rename_sponsors(self, chapters):
        ...
        for i, c in enumerate(chapters):
            ...
            if ("_was_cut" in c or "_categories" in c) and c["end_time"] - c[
                "start_time"
            ...
            if cats:
                ...
                if (
                    new_chapters
                    and "categories" in new_chapters[-1]
    ...
    def remove_chapters(
        self, filename, ranges_to_cut, concat_opts, force_keyframes=False
    ):
        in_file = filename
    ...
    def real_run_ffmpeg(
        self, input_path_opts, output_path_opts, *, expected_retcodes=(0,)
    ):
        self.check_version()
        ...
        def make_args(file, args, name, number):
            ...
            if name == "i":
                args.append("-i")
            return [encodeArgument(arg) for arg in args] + [
