...
download_tailwindcss() {
    ...
    test -n "${latest_version}" || return 1
    local latest_digest="$(./asfald-latest --get-hash "https://github.com/${owner}/${repo}/releases/download/v${latest_version}/${TW_BIN}")"
...
get_tailwindcss_version() {
    ...
    local help_output="$("${tailwindcss_bin}" --help)"
