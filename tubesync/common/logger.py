...
if (
    ...
    and "default" in settings.DATABASES.keys()
    and "_msgs" in settings.DATABASES.get("default", dict()).keys()
    and (_msgs := settings.DATABASES.get("default", dict()).pop("_msgs", False))
):
    for _spec in _msgs:
        try:
            _level, _msg = _spec
