...
def validYoutubeID(arg, /):
    ...
    valid_beginning = arg_str[0:2] in frozenset(
        (
            "PL",
            "UC",
            "UU",
    ...
    valid_ending = (arg_str[0:2] in frozenset(("PL",))) or (
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if nfo_path.exists():
                log.debug(f"not overwriting the existing path: {nfo_path}")
                return
            content = f"""\
<tvshow>
