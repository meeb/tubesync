...
def SQLTable(arg_table):
    ...
    valid_table_name = needle in new_tables and arg_table in db_tables(
        include_views=False
    )
...
class Command(BaseCommand):
    ...
    def _using_char(self, table_str, column_str="uuid", /):
        ...
        return column_str in [
            c.name
            for c in cols
            if c.data_type in char_types and c.display_size in char_sizes
        ]

    def _column_type(self, table_str, column_str="uuid", /):
        cols = db_columns(table_str)
        found = [
            f"{c.data_type}({c.display_size})"
            for c in cols
            if column_str.lower() == c.name.lower()
        ]
        if not found:
            return str()
        return found[0]
    ...
    def handle(self, *args, **options):
        ...
        db_is_mariadb = (
            hasattr(db.connection, "mysql_is_mariadb")
            and db.connection.is_usable()
        ...
        if options["uuid_columns"]:
            ...
            both_tables = self._using_char("sync_source", "uuid") and self._using_char(
                "sync_media", "uuid"
            )
            if not both_tables:
                if (
                    uuid_column_type_str
                    == self._column_type("sync_source", "uuid").lower()
                ):
                    log.info("The source table is already using a native UUID column.")
                elif (
                    uuid_column_type_str
                    == self._column_type("sync_media", "uuid").lower()
                ):
                    log.info("The media table is already using a native UUID column.")
                elif (
                    uuid_column_type_str
                    == self._column_type("sync_media", "source_id").lower()
                ):
                    log.info("The media table is already using a native UUID column.")
                else:
                    raise CommandError(
                ...
                fk_name_str = quote_name(
                    "sync_media_source_id_36827e1d_fk_sync_source_uuid"
                )
                ...
                schema.execute(
                    schema.sql_alter_column
                    % dict(
                        table=media_table_str,
                        changes=schema.sql_alter_column_not_null
                        % dict(
                            type=uuid_type_str,
        ...
        if table_names:
            ...
            at_30, err_30, out_30 = check_migration_status(
                "0030_alter_source_source_vcodec"
            )
            at_31, err_31, out_31 = check_migration_status(
                "0031_metadata_metadataformat"
            )
            at_31s, err_31s, out_31s = check_migration_status(
            ...
            should_delete = not (at_31s or after_31) and (at_30 or at_31)
            if not should_delete:
                raise CommandError(
                    _("Deleting metadata tables that are in use is not safe!")
                )

            for table in table_names:
                schema.execute(
                    schema.sql_delete_table
                    % dict(
