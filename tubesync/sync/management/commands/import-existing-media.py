...
class Command(BaseCommand):
    ...
    help = (
        "Scans download media directories for media not yet downloaded and ",
    ...
    def handle(self, *args, **options):
        ...
        for sourceroot, source in dirmap.items():
            media = list(
                Media.objects.filter(source=source, downloaded=False, skip=False)
            )
            if not media:
                log.info(f'Source "{source}" has no missing media')
                continue
            log.info(
                f'Scanning Source "{source}" directory for media to '
            ...
            for filepath, item in filemap.items():
                log.info(
                    f"Matched on-disk file: {filepath} "
                    f"to media item: {item.source} / {item}"
                )
                item.media_file.name = str(
                    Path(filepath).relative_to(item.media_file.storage.location)
                )
                ...
                if item.has_metadata:
                    metadata_date = timestamp_to_datetime(
