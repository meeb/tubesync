...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--source", action="store", required=True, help=_("Source UUID")
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError(f"Source does not exist with " f"UUID: {source_uuid}")
        ...
        log.info(
            f'Found source with UUID "{source.uuid}" with name '
