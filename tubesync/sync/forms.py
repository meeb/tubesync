...
class ValidateSourceForm(forms.Form):
    ...
    source_url = forms.URLField(
        ...
        widget=forms.URLInput(
            attrs={"placeholder": "https://www.youtube.com/@channelname"}
        ),
        **_assume_scheme,
    )
...
class ConfirmDeleteSourceForm(forms.Form):
    ...
    delete_media = forms.BooleanField(
        label=_("Also delete downloaded media"), required=False
    )
...
class JellyfinMediaServerForm(forms.Form):
    ...
    libraries = forms.CharField(
        label=_(
            f"Comma-separated list of {_media_server_type_label} library IDs to update"
        ),
        required=False,
    )
...
class PlexMediaServerForm(forms.Form):
    ...
    host = forms.CharField(
        label=_("Host name or IP address of the Plex server"), required=True
    )
    port = forms.IntegerField(
        label=_("Port number of the Plex server"), required=True, initial=32400
    )
    use_https = forms.BooleanField(
        label=_("Connect over HTTPS"),
        required=False,
    ...
    token = forms.CharField(label=_("Plex token"), required=True)
    libraries = forms.CharField(
        label=_(
            'Comma-separated list of Plex library IDs to update, such as "9" or "4,6"'
        )
    )
