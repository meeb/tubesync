...
class MediaServer:
    ...
    def make_request_args(
        self, uri="/", token_header=None, headers={}, token_param=None, params={}
    ):
        ...
        qs = urlencode(params)
        enable_verify = base_parts.scheme.endswith("s") and self.object.verify_https
...
class PlexMediaServer(MediaServer):
    ...
    HELP = _(
        "<p>To connect your TubeSync sevrer to your Plex Media Server you will "
    ...
    def make_request(self, uri="/", /, *, headers={}, params={}):
        url, kwargs = self.make_request_args(
            uri=uri, headers=headers, token_param="X-Plex-Token", params=params
        )
    ...
    def validate(self):
        ...
        if port < 1 or port > 65535:
            raise ValidationError(
                'Plex Media Server "port" must be between 1 ' "and 65535"
            )
        ...
        for position, library in enumerate(libraries):
            ...
            try:
                ...
                raise ValidationError(
                    f'Plex Media Server library ID "{library}" at '
        ...
        try:
            ...
            raise ValidationError(
                f"Failed to make a test connection to your Plex "
        ...
        if response.status_code != 200:
            ...
            if 400 <= response.status_code < 500:
                check_token = (
                    " A 4XX error could mean your access token is being "
                    "rejected. Check your token is correct."
                )
            raise ValidationError(
                f"Your Plex Media Server returned an invalid HTTP "
        ...
        try:
            ...
            raise ValidationError(
                f"Your Plex Media Server returned unexpected data, "
        ...
        for remote_library_id, remote_library_name in remote_libraries.items():
            remote_libraries_desc.append(
                f'"{remote_library_name}" with ID ' f'"{remote_library_id}"'
            )
        ...
        for library_id in libraries:
            ...
            if library_id not in remote_libraries:
                raise ValidationError(
                    f"One or more of your specified library IDs do "
        ...
        for library_id in libraries.split(","):
            ...
            if response.status_code != 200:
                raise MediaServerError(
                    f'Failed to refresh library "{library_id}" on '
...
class JellyfinMediaServer(MediaServer):
    ...
    HELP = _(
        "<p>To connect your TubeSync server to your Jellyfin Media Server, please enter the details below.</p>"
    ...
    def make_request(
        self, uri="/", /, *, headers={}, params={}, data={}, json=None, method="GET"
    ):
        assert method in {"GET", "POST"}, f"Unimplemented method: {method}"

        headers.update({"Content-Type": "application/json"})
        url, kwargs = self.make_request_args(
            uri=uri, token_header="X-Emby-Token", headers=headers, params=params
        )
    ...
    def validate(self):
        ...
        try:
            ...
            if port < 1 or port > 65535:
                raise ValidationError(
                    'Jellyfin Media Server "port" must be between 1 and 65535'
                )
        except (TypeError, ValueError):
        ...
        try:
            response = self.make_request(
                ...
                params={"Recursive": "true", "IncludeItemTypes": "CollectionFolder"},
            )
            if response.status_code != 200:
                raise ValidationError(
                    f"Failed to connect to Jellyfin server: {response.status_code}"
                )
            data = response.json()
        ...
        try:
            ...
            raise ValidationError(
                f"Jellyfin Media Server returned unexpected data, "
    ...
    def update(self):
        ...
        for library_id in map(str.strip, libraries):
            ...
            ):  # 204 No Content is expected for successful refresh
                raise MediaServerError(
                    f'Failed to refresh Jellyfin library "{library_id}", status code: {response.status_code}'
                )
        return True
