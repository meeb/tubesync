    ...
    with yt_dlp.YoutubeDL(opts) as y:
        try:
            ...
            try:
                ...
                raise YouTubeError(
                    f'Failed to extract channel ID for "{url}": {e}'
                ) from e
            else:
                return channel_id
...
def get_image_info(url):
    ...
    with yt_dlp.YoutubeDL(opts) as y:
        try:
            ...
            md = t[0]
            field_defaults = {
                f.attname: f.get_default() for f in md._meta.fields if f.has_default()
            }
...
def get_media_info(url, /, *, days=None, info_json=None):
    ...
    if days is not None:
        try:
            ...
            days = None
        start = f"yesterday-{days!s}days" if days else None
    opts = get_yt_opts()
    default_opts = yt_dlp.parse_options([]).options

    class NoDefaultValue:
        pass  # a unique Singleton, that may be checked for later
    ...
    try:
        ...
        paths.update({"infojson": user_set("infojson", paths, str(info_json_path))})
    ...
    if start:
        log.debug(
            f'get_media_info: used date range: {opts["daterange"]} for URL: {url}'
        )
    response = {}
    with yt_dlp.YoutubeDL(opts) as y:
        try:
            ...
            with yt_dlp.YoutubeDL(opts) as yy:
                try:
                    ...
                    raise YouTubeError(
                        f'Failed (again) to extract_info for "{url}": {ee}'
                    ) from ee
    ...
    if not response:
        raise YouTubeError(
            f'Failed to extract_info for "{url}": No metadata was '
...
def download_media(
    ...
    if isinstance(postprocessors, str):
        ...
        postprocessors = list(
            dict(_postprocessor_opts_parser(*val.split(":", 1)))
    ...
    pp_opts.exec_cmd.update(opts.get("exec_cmd", default_opts.exec_cmd))
    ...
    set_ffmpeg_codec = "modifychapters+ffmpeg" not in ytopts["postprocessor_args"]
    ...
    with yt_dlp.YoutubeDL(opts) as y:
        try:
            ...
            remove_unavailable_format = (
                settings.YOUTUBE_DL_SKIP_UNAVAILABLE_FORMAT
                and ": Requested format is not available." in str(e)
            )
            if remove_unavailable_format:
                raise FormatUnavailableError(
                    url, exc=e.__cause__, format=opts.get("format")
                ) from e
