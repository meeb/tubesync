...
def cleanup_completed_tasks():
    ...
    log.info(
        f"Deleting completed tasks older than {days_to_keep} days "
...
def schedule_indexing():
    ...
    for source in qs_gen(qs):
        previous_run = next_hour - timezone.timedelta(seconds=source.index_schedule)
        skip_source = (
            not source.is_active
            or source.target_schedule >= next_hour
            or (source.last_crawl and source.last_crawl >= previous_run)
        )
        if skip_source:
            continue
        try:
...
def wait_for_errors(model, /, *, queue_name=None, task_name=None):
    ...
    if queue_name:
        ...
        total_count += sum(
            [1 if contains_http429(q, k) else 0 for k in q.all_results()]
        )
...
def cleanup_old_media(durable=True):
    with atomic(durable=durable):
        for source in qs_gen(
            Source.objects.filter(delete_old_media=True, days_to_keep__gt=0)
        ):
            ...
            for media in qs_gen(mqs):
                log.info(
                    f"Deleting expired media: {source} / {media} "
...
def migrate_to_metadata(media_id):
    try:
        ...
        log.error(
            f"Task migrate_to_metadata(pk={media_id}) called but no "
    ...
    try:
        ...
        raise CancelExecution(
            _("no indexed data to migrate to metadata"), retry=False
        ) from e
...
def index_source(source_id):
    ...
    while len(videos) > 0:
        ...
        media_defaults["duration"] = (
            float(video.get(fields("duration", media), None) or 0) or None
        )
...
def download_source_images(source_id):
    ...
    try:
        ...
        log.error(
            f"Task download_source_images(pk={source_id}) called but no "
            f"source exists with ID: {source_id}"
        )
        raise CancelExecution(_("no such source"), retry=False) from e
    avatar, banner, thumbnail = source.get_image_url
    log.info(
        f"Thumbnail URL for source with ID: {source_id} / {source} "
        f"Avatar: {avatar} "
        f"Banner: {banner} "
...
def download_media_metadata(media_id):
    ...
    try:
        ...
        # Task triggered but the media no longer exists, do nothing
        log.error(
            f"Task download_media_metadata(pk={media_id}) called but no "
            f"media exists with ID: {media_id}"
        )
        raise CancelExecution(_("no such media"), retry=False) from e
    if media.manual_skip:
        log.info(
            f"Task for ID: {media_id} / {media} skipped, due to task being manually skipped."
        )
        return
    source = media.source
    wait_for_errors(
    ...
    try:
        ...
        log.info(
            f"Saved {len(media.metadata_dumps())} bytes of metadata for: "
...
def download_media_image(media_id, url):
    ...
    if (i.width > width) and (i.height > height):
        log.info(
            f"Resizing {i.width}x{i.height} thumbnail to " f"{width}x{height}: {url}"
        )
    ...
    copy_thumbnail = (
        media.downloaded and media.source.copy_thumbnails and media.thumb_file_exists
    )
    if copy_thumbnail:
        log.info(
            f"Copying media thumbnail from: {media.thumb.path} "
...
def on_complete_download_media_image(
    ...
    with huey_lock_task(
        ...
        try:
            ...
            if media.has_metadata:
                log.debug(
                    f'Scheduling a task to refresh metadata for: {media.key}: "{media.name}"'
                )
                TaskHistory.schedule(
                    refresh_formats,
                    str(media.pk),
...
@dynamic_retry(
    db_task,
    backoff_func=lambda n: (n * 3600) + 600,
    priority=50,
    retries=15,
    queue=Val(TaskQueue.LIMIT),
)
...
def rename_all_media_for_source(source_id):
    try:
        ...
        # Task triggered but the source no longer exists, do nothing
        log.error(
            f"Task rename_all_media_for_source(pk={source_id}) called but no "
            f"source exists with ID: {source_id}"
        )
        raise CancelExecution(_("no such source"), retry=False) from e
    ...
    create_rename_tasks = (
        source.directory and source.directory in rename_sources_setting
    ) or getattr(settings, "RENAME_ALL_SOURCES", False)
...
def save_all_media_for_source(source_id):
    ...
    try:
        ...
        # Task triggered but the source no longer exists, do nothing
        log.error(
            f"Task save_all_media_for_source(pk={source_id}) called but no "
            f"source exists with ID: {source_id}"
        )
        raise CancelExecution(_("no such source"), retry=False) from e
    ...
    saved_now = {
        str(media.pk) for media in qs_gen(save_qs) if str(media.pk) not in saved_later
    }
...
def delete_all_media_for_source(source_id, source_name, source_directory):
    ...
    delete_media.map({str(media.pk) for media in qs_gen(mqs)})
    with atomic(durable=True):
    ...
    remove = (source and source.delete_removed_media) or (
        directory_path / ".to_be_removed"
    ).is_file()
    if source:
        with atomic(durable=True):
            source.delete()
