...
def write_text_file(filepath, filedata):
    ...
    with NamedTemporaryFile(
        mode="wt", suffix=".tmp", prefix="", dir=filepath_dir, delete=False
    ) as f:
...
def _url_keys(arg_dict, filter_func):
    ...
    if isinstance(arg_dict, dict):
        for key, value in arg_dict.items():
            if "url" in key:
                result.update({key: filter_func(key=key, url=value)})
    return result
...
def filter_response(arg_dict, copy_arg=False):
    ...
    def drop_format_url(**kwargs):
        ...
        return (
            url
            and "://" in url
            and ("/ip/" in url or "ip=" in url or "/expire/" in url or "expire=" in url)
        )
    ...
    def drop_subtitles_url(**kwargs):
        url = kwargs["url"]
        return url and "://" in url and ("/expire/" in url or "&expire=" in url)
