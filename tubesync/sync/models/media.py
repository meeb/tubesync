...
class Media(models.Model):
    ...
    downloaded_hdr = models.BooleanField(
        _("downloaded hdr"), default=False, help_text=_("Downloaded media has HDR")
    )
    ...
    def save(
            ...
            for filepath in fp_list:
                if filepath.exists():
                    self.media_file.name = str(
                        filepath.relative_to(self.media_file.storage.location)
                    )
                    self.skip = False
        ...
        update_md = self.has_metadata and (
            update_fields is None or "metadata" in update_fields
        )
        if update_md:
                ...
                if audio_format and video_format:
                    ...
                    for fmt in reversed(list(self.iter_formats())):
                        select_fmt = (
                            ...
                            and fmt.get("acodec")
                            and fmt.get("vcodec")
                            and self.source.can_fallback
                            and (
                                (
                                    self.source.fallback
                                    == Val(Fallback.NEXT_BEST_RESOLUTION)
                                )
                                or (
                                    self.source.fallback == Val(Fallback.REQUIRE_CODEC)
                                    and self.source.source_vcodec == fmt.get("vcodec")
                                )
                                or (
                                    self.source.fallback == Val(Fallback.REQUIRE_HD)
                                    and (fmt.get("height") or 0) >= fallback_hd_cutoff
                                )
                            )
                        )
                        if select_fmt:
    ...
    def get_display_format(self, format_str):
        ...
        if self.downloaded:
            ...
            if not self.downloaded_video_codec and not self.downloaded_audio_codec:
    ...
    def has_metadata(self):
        ...
        if not result:
            return False
        value = self.get_metadata_first_value(
            (
                "id",
                "display_id",
                "channel_id",
    ...
    def reduce_data(self):
        ...
        try:
            ...
            filtered_data["_reduce_data_ran_at"] = round(
                (now - self.posix_epoch).total_seconds()
            )
            ...
            if old_mdl > new_mdl:
                ...
                log.info(
                    f"{self.key}: metadata compacted by {delta:,} characters ({old_mdl:,} -> {new_mdl:,})"
                )
            new_mdl = len(filtered_json)
            if old_mdl > new_mdl:
                delta = old_mdl - new_mdl
                log.info(
                    f"{self.key}: metadata reduced by {delta:,} characters ({old_mdl:,} -> {new_mdl:,})"
                )
    ...
    def metadata_title(self):
        return self.get_metadata_first_value(
            (
                "fulltitle",
                "title",
            ),
    ...
    def ts_to_dt(self, /, timestamp):
        try:
            ...
            log.warn(
                f'Could not compute published from timestamp for: {self.source} / {self} with "{e}"'
            )
            pass
        else:
    ...
    def nfoxml(self):
        ...
        nfo.append(
            _nfo_element(
                nfo,
                "showtitle",
                clean_emoji(str(self.source.name).strip()),
            )
        )
        ...
        nfo.append(
            _nfo_element(
                ...
                (
                    ...
                    else str(self.upload_date.year if self.upload_date else "")
                ),
            )
        )
        ...
        nfo.append(
            _nfo_element(
                nfo,
                "plot",
                clean_emoji(str(self.description).strip()),
            )
        )
        ...
        nfo.append(
            _nfo_element(
                nfo,
                "thumb",
                self.thumbname if self.source.copy_thumbnails else "",
            )
        )
        ...
        nfo.append(
            _nfo_element(
                nfo,
                "studio",
                clean_emoji(str(self.uploader).strip()),
            )
        )
        ...
        nfo.append(
            _nfo_element(
                nfo,
                "aired",
                upload_date.strftime("%Y-%m-%d") if upload_date else "",
            )
        )
    ...
    def download_media(self):
        ...
        if not format_str:
            raise NoFormatException(
                f'Cannot download, media "{self.pk}" ({self}) has '
        ...
        download_youtube_media(
            ...
            self.source.sponsorblock_categories.expand_choices,
            self.source.embed_thumbnail,
            self.source.embed_metadata,
            self.source.enable_sponsorblock,
            self.source.write_subtitles,
            self.source.auto_subtitles,
            self.source.sub_langs,
        )
    ...
    def index_metadata(self):
        ...
        if not callable(indexer):
            raise Exception(
                f'Media with source type f"{self.source.source_type}" '
                f"has no indexer"
            )
        response = indexer(self.url)
        no_formats_available = (
            not response
            or "formats" not in response.keys()
            or 0 == len(response["formats"])
        )
        ...
        if self.downloaded and self.media_file:
            ...
            if old_video_path.exists() and not new_video_path.exists():
                ...
                if "{key}" in media_format:
                    fuzzy_paths = list(
                        top_dir_path.rglob("*" + glob_quote(str(self.key)) + "*")
                    )
                ...
                if new_video_path.exists():
                    ...
                    self.media_file.name = str(
                        new_video_path.relative_to(self.media_file.storage.location)
                    )
                    self.skip = False
                    ...
                    for other_path in other_paths:
                        ...
                        if new_file_path == other_path:
                            continue
                        log.debug(
                            f"Considering replace for: {self!s}\n\t{other_path!s}\n\t{new_file_path!s}"
                        )
                    ...
                    for fuzzy_path in fuzzy_paths:
                        if not fuzzy_path.exists():
                            continue
                        fuzzy_prefix_path, fuzzy_stem = directory_and_stem(
                            fuzzy_path, True
                        )
                        ...
                        if new_file_path == fuzzy_path:
                            continue
                        log.debug(
                            f"Considering rename for: {self!s}\n\t{fuzzy_path!s}\n\t{new_file_path!s}"
                        )
                        # it quite possibly was renamed already
                        if not (
                            new_video_path.samefile(fuzzy_path)
                            or new_file_path.exists()
                        ):
                    ...
                    if self.source.write_nfo and self.source.copy_thumbnails:
                        write_text_file(
                            new_prefix_path / self.nfopath.name, self.nfoxml
                        )
