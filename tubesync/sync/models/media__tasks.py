...
def copy_thumbnail(self):
    ...
    if not self.thumb_file_exists:
        return
    log.info(
        "Copying media thumbnail" f" from: {self.thumb.path}" f" to: {self.thumbpath}"
    )
...
def download_checklist(self, skip_checks=False):
    ...
    downloaded_file_exists = (
        media.downloaded
        and media.has_metadata
        and (media.media_file_exists or media.filepath.exists())
    )
...
def download_finished(self, format_str, container, downloaded_filepath=None):
    ...
    log.info(
        f"Successfully downloaded media: {media} (UUID: {media.pk}) to: "
        f'"{filepath}"'
    )
...
def failed_format(self, format_str, /, *, cause=None, exc=None):
    ...
    new_formats = [f for f in formats if f.get("format_id") not in (t[0],)]
...
def refresh_formats(self):
    ...
    if attempted_seconds:
        ...
        if (now - attempted_dt) < timezone.timedelta(
            seconds=self.source.index_schedule // 3
        ):
    ...
    return (
        True,
        False,
        "updated formats{s}{a}{j}{t}".format(**{k: fmt_dict[k] for k in "sajt"}),
    )
...
def write_nfo_file(self):
    ...
    try:
        ...
        msg = "A permissions problem occured when writing" " the new media NFO file: {}"
        log.exception(msg, e)
