...
class Source(db.models.Model):
    ...
    media_format = db.models.CharField(
        ...
        help_text=_(
            "File format to use for saving files, detailed options at bottom of page."
        ),
    )
    ...
    download_media = db.models.BooleanField(
        ...
        default=True,
        help_text=_(
            "Download media from this source, if not selected the source will only be indexed"
        ),
    )
    ...
    filter_text_invert = db.models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Invert filter string regex match, skip any matching titles when selected"
        ),
    )
    ...
    source_vcodec = db.models.CharField(
        ...
        help_text=_(
            'Source video codec, desired video encoding format to download (ignored if "resolution" is audio only)'
        ),
    )
    ...
    fallback = db.models.CharField(
        ...
        help_text=_(
            "What do do when media in your source resolution and codecs is not available"
        ),
    )
    copy_channel_images = db.models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Copy channel banner and avatar. These may be detected and used by some media servers"
        ),
    )
    copy_thumbnails = db.models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Copy thumbnails with the media, these may be detected and used by some media servers"
        ),
    )
    write_nfo = db.models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Write an NFO file in XML with the media info, these may be detected and used by some media servers"
        ),
    )
    write_json = db.models.BooleanField(
        ...
        default=False,
        help_text=_(
            "Write a JSON file with the media info, these may be detected and used by some media servers"
        ),
    )
    ...
    sub_langs = db.models.CharField(
        ...
        default="en",
        help_text=_(
            "List of subtitles langs to download, comma-separated. Example: en,fr or all,-fr,-live_chat"
        ),
        validators=[
            RegexValidator(
                regex=r"^(\-?[\_\.a-zA-Z-]+(,|$))+",
                message=_(
                    "Subtitle langs must be a comma-separated list of langs. example: en,fr or all,-fr,-live_chat"
                ),
            ),
        ],
    )
    ...
    sponsorblock_categories = CommaSepChoiceField(
        ...
        default="all",
        help_text=_(
            "Select the SponsorBlock categories that you wish to be removed from downloaded videos."
        ),
    )
    ...
    @property
    def is_active(self):
        active = self.download_media or self.index_streams or self.index_videos
    ...
    def get_index_url(self, url_type, /):
        return self.__class__.create_index_url(
            self.source_type, url_key=self.key, url_type=url_type
        )
