...
ARG TARGETARCH
RUN --mount=type=cache,id=apt-lib-cache-${TARGETARCH},sharing=locked,target=/apt-lib-cache \
    --mount=type=cache,id=apt-cache-cache,sharing=locked,target=/apt-cache-cache \
